class Achievement {
	...
	get desc() {
		let descIsFunction = (typeof ACH_DATA.descs[this.name] == "function")
		return (
			this.title +
			"\n" +
			(
	...
	grant() {
		if (!player.achievements.includes(this.name) && getAllAchievements().includes(this.name)) {
			...
			notifier.success("Achievement " + this.name + " gotten: " + ACH_DATA.names[this.name]);
		}
	}

	remove() {
		player.achievements = player.achievements.filter(item => item !== this.name)
	}
