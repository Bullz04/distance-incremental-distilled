function calcAcceleration(){
	...
	if (tmp.ach) if (tmp.ach[51].has && player.distance.gte(DISTANCES.uni)) tmp.acc = tmp.acc.times(200)
	if (tmp.ach) if (tmp.ach[105].has) tmp.acc = tmp.acc.times(4);
	if (tmp.rf) tmp.acc = tmp.acc.times(getDistilledFuelEff())
...
function calcMaxVelocity(){
	...
	if (tmp.ach) if (tmp.ach[51].has && player.distance.gte(DISTANCES.uni)) tmp.maxVel = tmp.maxVel.times(200);
	...
	if (tmp.rf) tmp.maxVel = tmp.maxVel.times(getDistilledFuelEff())
