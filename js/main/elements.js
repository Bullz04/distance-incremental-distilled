...
function updateRocketsHTML(){
	if (player.tab == "rockets") {
		...
		tmp.el.rocketsEff.setTxt(showNum(getDistilledRocketEffect()));
		...
		tmp.el.rfEff.setTxt(showNum(getDistilledFuelEff()));
		tmp.el.rfName.setTxt("Rocket Fuel");
		tmp.el.rf2.setTxt(showNum(getFuelEff2()));//adds x additional rockets
	}
}
	...
	if (player.tab == "achievements") {
		...
		for (let r = 1; r <= ACH_DATA.rows; r++) {
			...
			for (let c = 1; c <= ACH_DATA.cols; c++) {
				...
				if (typeof tmp.ach[id].desc == "function") {
...
function updateRobotsHTML(){
	tmp.el["Ach46RewardEffIntelligence"].setHTML(
		(tmp.ach[46].has) ? "Achievement 46 reward: x" + showNum(ach46Eff("Intelligence")) + " intelligence gain <br>" : ""
	)
...
function updateTimeReversalHTML(){
	if (player.tab == "tr") {
		...
		for (let i = 1; i <= TR_UPG_AMT; i++) {
			...
			if (upg.current !== undefined && (i > 15 ? modeActive("extreme") : true)) effDisp += "<br>Currently:<br><span class='grossminitxt' style=\"font-size: 13px;\">" + upg.disp(upg.current()) + "</span>"
			tmp.el["tr" + i].setHTML(desc + "<br>Cost: " + showNum(upg.cost()) + " Time Cubes." + effDisp);
	...
	if (player.tab == "statistics") {
		...
		if (statTab == "rankTiers") {
			...
			for (let i=0;i<Object.keys(RANK_DESCS).length;i++) {
				...
				if (tmp.el["rankEff"+(i+1)]) tmp.el["rankEff"+(i+1)].setTxt(showNum(getRankEffects(i+1)));
			}
