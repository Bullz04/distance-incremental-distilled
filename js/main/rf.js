...
function getFuelEff2() {
	let eff = player.rf.plus(getFreeFuel()).pow(4);
	eff = eff.times(EN.pow(1.2, player.rf.root(2)))
...
function updateTempRF() {
	...
	if (!tmp.rf.onReset) tmp.rf.onReset = function (prev) {
		...
		else if (tmp.ach[58].has) player.rockets = prev.rockets.div(2).max(10).floor();
...
function getRocketFuelFP() {
	let eff = new ExpantaNum(2.5)
	if (tmp.pathogens && player.pathogens.unl) eff = eff.times(tmp.pathogens[7].eff())
	return eff
}