...
function getDistilledRocketEffect() {
	let r = player.rockets.plus(getFuelEff2());
	let eff = r.plus(1).pow(2);
	return eff
}
...
function updateTempRockets() {
	...
	let r = player.rockets;
	if (mlt5) r = r.min(0);
	tmp.rockets.accPow = getDistilledRocketEffect();
	tmp.rockets.mvPow = getDistilledRocketEffect();
	tmp.rockets.accEnPow = getDistilledRocketEffect();
