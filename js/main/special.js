function loadTempFeatures() {
	tmp.features = {
		...
		"time reversal": new Feature({
			name: "time reversal",
			req: function() { return new ExpantaNum(TR_UNL) },
			res: "distance",
			display: formatDistance,
			reached: function() { return player.tr.unl },
			progress: function () {
				if (player.options.featPerc=="logarithm") {
					return player.distance.max(1).log10().div(new ExpantaNum(TR_UNL).log10());
				} else return ExpantaNum.div(TR_UNL, player.distance).pow(-1)
			}
		}),
